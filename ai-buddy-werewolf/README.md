...
```bash
npm test        # vitest (役職配布/秘密分離/150秒の独立AI討論・名指し応答・期限終了/親密度補正・勾配集計/初日占い/襲撃統合/勝敗/リプレイ復元/公開Lab復元/愛言葉正規化/引継ぎ整合性/モック完走 など68件)
...
2. **ゲーム画面**: 既定は150秒の自由討論。初日は抽選された2人が先に弁明し、その後は各バディが個別に考えて同時並行で話す。名指しされた相手は反論し、指名質問は対象が個別回答する。時間内に1日1回の相談(疑い/質問/確定情報共有/スキル対象/立ち回り)を送り、終了後の裁判で処刑したい相手を選ぶ(最終投票はバディが決める)。狼なら夜に襲撃提案
