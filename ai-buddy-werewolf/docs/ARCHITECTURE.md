...
├── supabase/functions/ai-buddy-lab/ # 公開Web Labの合言葉認証 + OpenRouter中継
└── docs/
```

実行形態は2つある。

1. **Node版**: `apps/web → HTTP → apps/server → ai-engine → game-core → shared`。試合JSONは `data/` へ保存する
2. **公開静的Web Lab版**: `apps/web/runtime/browserBackend → ai-engine/browser → game-core → shared`。試合・編集内容はブラウザの `localStorage` へ保存し、Live AIだけをEdge Functionへ送る

依存方向(逆流禁止):

```
Node:    apps/web ──HTTP──▶ apps/server ──▶ ai-engine ──▶ game-core ──▶ shared
Static:  apps/web/browserBackend ─────────▶ ai-engine/browser ──▶ game-core ──▶ shared
                                                │
                                                └──Liveのみ──▶ Supabase Edge Function ──▶ OpenRouter
```
...
| `apps/web` | UI、Node APIクライアント、公開Lab用ブラウザ実行アダプター・ローカル保存 | 独自のルール判定（必ずgame-coreを使う） |
| `supabase/functions/ai-buddy-lab` | 合言葉SHA-256照合、許可Origin/モデル制限、構造化出力、OpenRouter中継 | ゲーム状態・試合保存・平文合言葉 |
...
- 含まれる: 公開ログ（公開された役職の名乗りを含む） / 自分の役職 / 狼仲間(狼のみ) / **共有済み**確定情報 / 自分の主人の助言 / 自分の能力・人格 / 自分の過去評価
- 含まれない: 他組の本当の役職 / 未共有の占い・霊媒結果 / 他の主人の助言（名乗り方の相談も含む） / 他AIの内部スコア / GM役職表 / シード(将来の乱数結果)

役職を名乗る処理は、秘密の `role_claim` 助言と公開の `role_declared` イベントを分ける。公開イベントには本人が名乗った役職だけを保存し、本当か偽りかは保存しない。真役職との比較は試合終了後のリプレイでのみ組み立てる。これにより、発言ログを圧縮しても過去の名乗りがAIの公開情報から消えない。

通常のPlay Testでは、進行中のリプレイ要求に `lab=1` を付けても内部データを返さない。進行中の内部分析を許可するのは、試合作成時から `mode: "lab"` だった試合だけである。またゲーム画面の助言メニューは現在のグローバル設定ではなく、その試合の `configSnapshot.advice` を使う。これにより、古い試合を再開しても画面とゲームコアの検証契約が食い違わない。
...
- `LabProxyProvider`: 公開Web LabからSupabase Edge Function経由でOpenRouterを呼ぶブラウザ用実装。APIキーはEdge側だけで保持し、合言葉・Origin・モデルallowlistを検証する
...
`AiEngine.run` / `BrowserAiEngine.run` が全コール共通で「レイテンシー・トークン・原価(単価×トークン)・リトライ・JSON失敗・エラー」を `AiCallRecord` に記録し、失敗時は**モックへ決定論的フォールバック**する。Edge側でもJSON Schemaをプロンプトと `response_format` の両方で要求し、ブラウザ側のZod検証に失敗した場合だけ設定回数まで再試行する。

## 永続化とリプレイ

- 保存先: `data/matches/<matchId>.json`(スキーマ = `MatchRecord`: メタ + `configSnapshot`(設定・プロンプトバージョン込み) + `events` + `aiCalls`)
- 公開Web Lab: `localStorage` の `aibw.lab.*` 名前空間に同じ `MatchRecord` と設定差分を保存。サーバーへ試合データを永続化しない
...
- Web Labから書き出す `ai-buddy-werewolf-mobile-handoff` はルール/人格/モデル設定/プロンプトをSHA-256付きで固定する。Flutterへプロンプトを埋め込まず、バックエンドへ取り込む契約は [MOBILE_HANDOFF.md](MOBILE_HANDOFF.md) を正とする
