...
- 主人の助言は直接命令ではないが、**親密度が高いほど、バディは自分の推理と違っても主人を信じる選択を優先する**。これは正解率ではなく関係性を表す
...
- 確定情報は親密度補正の対象外(事実として扱う)。ただし使い方(公開/温存)はAIが決める
- 人格(名前・口調・キャラ性)は表現にのみ影響し、推論力・虚言力・親密度と分離する
- 3能力は「10ポイントごとに新観点/新技術をアンロック + 中間ポイントで精度向上」という構造。虚言力が高くても必ず騙せるわけではない
- 親密度MAXでは主人を強く優先するが、確定情報との矛盾・不可能な対象・極端な評価差では拒否でき、100%服従にはしない
...
| 討論 | 全プリセットを2幕にする。初日は抽選された2人の弁明→残り全員の焦点評価→主人の相談→応答討論。指名質問は質問者→対象の単独回答→質問者の受け止め→周囲最大2組の反応 | `firstDayFocusCount`, `discussionRounds` とプリセット |
...
- 親密度別に主人の意見が反映される体感。高親密度ではAI自身の候補と違っても主人を優先することを成功とし、単純な一致率の最大化・最小化では評価しない
- 親密度補正 maxBonus の最終値(現在のQuick裁判主候補32。差が最大になる値は主人案の順位に依存するため、主指標は高親密度で残る非服従余白。標準実験では32が全定義で19%以上を維持し、40はAI第2候補で9.2%まで低下。40は服従側の対照条件とする)
