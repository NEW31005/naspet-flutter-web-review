...
- ゲームコア: 役職配布(市民/占い師/騎士/霊媒師/狼憑き)・150秒の時間制討論（`opening → awaiting_master_advice → response`、総時間の40%と発言枠1件以上をresponseへ予約、相談待ち中は時計停止、個別AI処理、名指し反論、質問→回答→追加反応、期限後の発言破棄）・裁判(意思表示→AI投票)・夜(占い/護衛/霊媒/襲撃統合)・勝敗・同票処理・イベントソーシング・リプレイ復元・フェーズ巻き戻し
...
- 助言6種(主観疑い/質問指定/確定情報共有/スキル対象提案/役職を名乗る相談/立ち回り)、1日1回制限
- 標準9組プリセット（狼憑き2・占い師1・騎士1・霊媒師1・市民4）。役職の名乗りは主人の秘密提案とバディの公開宣言を別イベントにし、真偽は試合後だけ表示
...
- 自動テスト128件（5役職・役職を名乗る相談/公開宣言/試合後真偽・初日焦点2人の弁明・討論3段階・相談中の時計停止/再開・15秒/5発言でも主人相談とresponseを保証・同時生成中の無関係発言を名指しへの返答と誤認しない回帰・指名質問の回答・時間切れ終了・Play Testの内部リプレイ境界・公開Labのブラウザ内完走/復元・Live AIと本格9組の初期選択・旧試合の凍結助言・旧v1引継ぎ互換・日本語を含む愛言葉正規化・複数の占い・護衛・霊媒・初日占い・秘密分離を含む）/ ESLint / strict TypeScript

## 未実装の範囲

- 現在の5役職以外の追加役職、役職種類の動的追加
...
- 進行中の内部リプレイは、試合作成時からLabモードだった試合だけ許可する。Play TestをURLやクエリだけでLabへ昇格させない
- 再開試合の助言UIは現在設定ではなく、試合作成時の `configSnapshot.advice` を表示する
...
3. 追加役職（現在の占い師・騎士・霊媒師以外）。既存と同じく `Role`・夜処理・秘密Fact・役職別プロンプトを一体で追加する
